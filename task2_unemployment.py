...
# --- 9 Key Stats Summary---
print(f"\n{'=*50'}")
print("📊 Key Finding Summray")
print(f"{'='*50}")
print(f"Pre-COVID avg unemployment  : {pre_covid:.2f}%")
print(f"During-COVID lockdown avg   : {dur_covid:.2f}%")
print(f"Post-COVID lockdown avg     : {post_covid:.2f}%")
highest = state_avg.idxmax()
lowest  = state_avg.idxmin()
print(f"Highest unenployment State   : {highest}({state_avg.max():.1f}%)")
print(f"Lowest unenployment State   : {lowest}({state_avg.min():.1f}%)")
ru = area_monthly[area_monthly['Area'] == 'Rural']['UnempRate'].mean()
ur = area_monthly[area_monthly['Area'] == 'Urban']['UnempRate'].mean()
print(f"Rural avg unemployment    : {ru:.2f}%")
print(f"Urban avg unemployment    : {ur:.2f}%")
print(f"\n ✅ TASK 2 COMPLETE - Unemployment Analysis with python")